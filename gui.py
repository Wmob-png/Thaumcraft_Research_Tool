...
from aspects import PRIMALS, aspect_parents, get_all_connections
...
class TilemapLoader:
    ...
    def get_tile(self, col, row):
        ...
        if x + self.tile_size > self.tilemap.get_width() or y + self.tile_size > self.tilemap.get_height():
            return None
...
class HexBoard:
    ...
    def undo(self):
        if not self.history:
            ...
            return False
        self.redo_stack.append({pos: dict(cell) for pos, cell in self.grid.items()})
    ...
    def redo(self):
        if not self.redo_stack:
            ...
            return False
        self.history.append({pos: dict(cell) for pos, cell in self.grid.items()})
    ...
    def _rebuild_connected_cache(self):
        ...
        for pos, cell in self.grid.items():
            if not cell.get("aspect") or cell.get("void", False):
                continue
            if cell.get("player_placed", False) and is_edge_position(self.grid, pos, self.ring_count):
        ...
        while queue:
            ...
            for neighbor_pos in self.get_hex_neighbors(current):
                ...
                if neighbor_asp and neighbor_asp in ASPECT_GRAPH.get(current_asp, set()):
    ...
    def _load_parchment(self):
        try:
            self.parchment_base = pygame.image.load(asset_path("parchment3.png")).convert_alpha()
        except Exception:
            self.parchment_base = None
        ...
        for row_i, row in enumerate(left_aspects):
            for col_i, asp in enumerate(row):
                if asp:
                    ...
                    self.aspect_buttons_left.append((pygame.Rect(x, y, ICON_SIZE, ICON_SIZE), asp))
        ...
        for row_i, row in enumerate(right_aspects):
            for col_i, asp in enumerate(row):
                if asp:
                    ...
                    y = y_start + row_i * spacing
                    self.aspect_buttons_right.append((pygame.Rect(x, y, ICON_SIZE, ICON_SIZE), asp))
    ...
    def draw_lightning_chains(self, screen):
        ...
        for pos, cell in self.grid.items():
            ...
            for npos in self.get_hex_neighbors(pos):
                ...
                if ncell["void"] or not ncell["aspect"] or not self.is_position_active(npos):
                    continue
                ...
                if self.are_aspects_connected(cell["aspect"], ncell["aspect"]):
                    ...
                    self._draw_lightning(screen, x1, y1, x2, y2, hash(pair) + self.lightning_seed, scale)
    ...
    def _draw_lightning(self, screen, x1, y1, x2, y2, seed, scale):
        ...
        for i in range(1, segments):
            ...
            points.append((x1 + dx * t + perp_x * offset, y1 + dy * t + perp_y * offset))
        points.append((x2, y2))

        for i in range(len(points) - 1):
            pygame.draw.line(screen, CHAIN_GLOW_COLOR, points[i], points[i + 1], max(1, int(4 * scale)))
            pygame.draw.line(screen, CHAIN_COLOR, points[i], points[i + 1], max(1, int(2 * scale)))
            pygame.draw.line(screen, (220, 240, 255), points[i], points[i + 1], 1)
    ...
    def _draw_progress_bar(self, screen, font):
        ...
        pct_surf = font.render(f"{int(progress * 100)}%", True, (255, 255, 255))
        ...
        bar_sx = sx + pad
        bar_sy = sy + pad + self.scale_value(title_surf.get_height()) + self.scale_value(8)
        ...
        title_scaled = pygame.transform.smoothscale(
            title_surf,
            (self.scale_value(title_surf.get_width()), self.scale_value(title_surf.get_height())),
        )
        screen.blit(title_scaled, title_scaled.get_rect(centerx=sx + sw // 2, top=sy + pad))

        pygame.draw.rect(screen, (50, 50, 50), (bar_sx, bar_sy, bar_sw, bar_sh))
        pygame.draw.rect(screen, (100, 100, 100), (bar_sx, bar_sy, bar_sw, bar_sh), 1)
        ...
        if fill_w > 0:
            ...
            pygame.draw.rect(screen, fill_color, (bar_sx, bar_sy, fill_w, bar_sh))

            if not is_done and progress < 1.0:
                edge_x = bar_sx + fill_w - 1
                pygame.draw.line(screen, (220, 240, 255), (edge_x, bar_sy), (edge_x, bar_sy + bar_sh - 1))

        pct_y = bar_sy + bar_sh + self.scale_value(6)
        pct_scaled = pygame.transform.smoothscale(
            pct_surf,
            (self.scale_value(pct_surf.get_width()), self.scale_value(pct_surf.get_height())),
        )
        screen.blit(pct_scaled, pct_scaled.get_rect(centerx=sx + sw // 2, top=pct_y))

        if msg_surf:
            msg_y = pct_y + self.scale_value(pct_surf.get_height()) + self.scale_value(6)
            msg_scaled = pygame.transform.smoothscale(
                msg_surf,
                (self.scale_value(msg_surf.get_width()), self.scale_value(msg_surf.get_height())),
            )
            screen.blit(msg_scaled, msg_scaled.get_rect(centerx=sx + sw // 2, top=msg_y))
    ...
    def draw(self, screen, font):
        ...
        if self.parchment_base:
            pw = self.scale_value(self.parchment_base.get_width() * PARCHMENT_SCALE)
            ph = self.scale_value(self.parchment_base.get_height() * PARCHMENT_SCALE)
            parchment = pygame.transform.smoothscale(self.parchment_base, (pw, ph))
        ...
        for (q, r), cell in self.grid.items():
            ...
            if self.hex_idle:
                img = self.hex_hover if is_hovered and self.hex_hover else self.hex_idle
        ...
        for (q, r), cell in self.grid.items():
            ...
            if aspect:
                ...
                icon = self._get_icon(aspect, icon_size, dimmed=not is_active)
                if icon:
    ...
    def _draw_status_bar(self, screen, font):
        ...
        pygame.draw.rect(surf, (100, 100, 100), (0, 0, bg.width, bg.height), 1)
        screen.blit(surf, bg)
    ...
    def _draw_hex_tooltip(self, screen, font, mx, my):
        ...
        screen.blit(font.render(aspect, True, (255, 255, 255)), (tx + TOOLTIP_ICON_SIZE + 14, ty + 10))

        if parents:
            screen.blit(font.render("=", True, (180, 180, 180)), (tx + 10, ty + 34))
            px = tx + 28
            for p in parents:
            ...
            screen.blit(font.render("Primal", True, (200, 180, 100)), (tx + 10, ty + 34))
    ...
    def draw_sidebars(self, screen, font):
        ...
        for base_rect, asp in self.aspect_buttons_left + self.aspect_buttons_right:
            ...
            if icon:
                ...
                screen.blit(font.render(asp[:3], True, (255, 255, 0)), (sx + 2, sy + 4))

            if is_hovered and not is_selected:
                pygame.draw.rect(screen, (255, 255, 255), screen_rect.inflate(2, 2), 1)

        if self.hovered_aspect and self.show_tooltips:
            self._draw_aspect_tooltip(screen, font, mx, my, self.hovered_aspect)
    ...
    def _draw_aspect_tooltip(self, screen, font, mx, my, aspect):
        ...
        y = ty + 32
        if parents:
            screen.blit(font.render("=", True, (180, 180, 180)), (tx + 10, y))
            px = tx + 28
            for p in parents:
                picon = self._get_icon(p, TOOLTIP_ICON_SIZE, False)
                if picon:
            ...
            screen.blit(font.render("Primal", True, (200, 180, 100)), (tx + 10, y))

        screen.blit(font.render(f"Connects: {len(connections)}", True, (150, 150, 150)), (tx + 10, ty + 56))
    ...
    def toggle_oi_chaining(self):
        ...
        self.set_status(f"IO Chain: {'ON' if self.oi_chaining else 'OFF'}")

    def toggle_center_priority(self):
        self.center_priority = not self.center_priority
        self.set_status(f"Center Priority: {'ON' if self.center_priority else 'OFF'}")

    def clear_board(self):
        self.save_state()
        cleared = sum(1 for c in self.grid.values() if c["aspect"] and not c.get("player_placed"))
        for cell in self.grid.values():
            if cell["aspect"] and not cell.get("player_placed"):
                cell["aspect"] = None
        self._connected_cache_dirty = True
        self.update_solved_state()
        self.set_status(f"Cleared {cleared} placements" if cleared else "Nothing to clear")
    ...
    def toggle_coords(self):
        ...
        self.set_status(f"Coordinates: {'ON' if self.show_coords else 'OFF'}")

    def toggle_tooltips(self):
        self.show_tooltips = not self.show_tooltips
        self.set_status(f"Tooltips: {'ON' if self.show_tooltips else 'OFF'}")
    ...
    def solve_board(self):
        ...
        def on_event(event, data):
            if event == "complete":
                ...
                self.set_status(f"Solved! {c} placements | Grade: {g} ({s})")
            elif event == "error":
        ...
        def run():
            try:
                result = solve_with_center_priority(
                    ...
                    io_chaining_enabled=self.oi_chaining,
                    callback=on_event,
    ...
    def debug_dump(self):
        debug_output_board(self.grid, self.ring_count, prefer_oi=self.oi_chaining)
