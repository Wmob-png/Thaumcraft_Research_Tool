...
from gif_loader import CatButtonManager
from gui import HexBoard
from resources import asset_path, resource_path
from solver import debug_output_board
...
cat_button = CatButtonManager(switch_chance=0.05)
...
def draw_board_size_menu():
    ...
    subtitle = small_font.render(
        "(Press 1, 2, or 3 to quick-select)",
        True,
        (150, 150, 150),
    )
...
CLEAR_IMG_FALLBACK = load_icon("brain.png")
...
BASE_BUTTONS_Y = BASE_HEIGHT - BASE_BUTTON_H - 10
...
def recompute_button_rects():
    ...
    credits_text_w, credits_text_h = small_font.size("Credits")
    credits_pad_x, credits_pad_y = 12, 6
    credits_w = int((credits_text_w + credits_pad_x * 2) * scale)
    credits_h = int((credits_text_h + credits_pad_y * 2) * scale)
    credits_button = pygame.Rect(0, WINDOW_HEIGHT - credits_h, credits_w, credits_h)
...
while running:
    ...
    current_solving = board.solve_running
    current_solved = board.is_solved

    if was_solving and not current_solving and current_solved:
        if cat_button.loaded:
            cat_button.trigger_cheer()

    was_solving = current_solving

    if cat_button.loaded:
        cat_button.update(dt_ms)
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_p:
                try:
                    prefer_oi = getattr(board, "oi_chaining", False)
                    debug_output_board(board.grid, board.ring_count, prefer_oi=prefer_oi)
                except Exception:
                    pass
                continue
    ...
    visually_active = not board.show_credits

    if IO_CENTER_IMG:
        draw_centered_icon(
            io_button,
            IO_CENTER_IMG,
            active=board.oi_chaining and visually_active,
            scale_mult=1.8,
        )
    if hovered_button == "io" and solve_active and visually_active:
        draw_button_tooltip(screen, io_button, button_tooltips["io"])

    if SIZE_IMG_1:
        flash = size_flash_frames > 0
        size_icon = SIZE_IMG_1
        if hovered_button == "size" and solve_active and visually_active:
            if SIZE_IMG_2:
    ...
    if cat_button.loaded:
        cat_button.draw_in_button(
            screen,
            clear_button,
            active=visually_active,
            scale_mult=1.3,
        )
    elif CLEAR_IMG_FALLBACK:
        draw_centered_icon(
            clear_button,
            CLEAR_IMG_FALLBACK,
            active=visually_active,
    ...
    if hovered_button == "clear" and solve_active and visually_active:
        draw_button_tooltip(screen, clear_button, button_tooltips["clear"])

    if SOLVE_IMG:
        if board.solve_running:
            draw_centered_icon(solve_button, SOLVE_IMG, active=True, scale_mult=1.5)
        else:
            draw_centered_icon(solve_button, SOLVE_IMG, active=visually_active, scale_mult=1.3)
    if hovered_button == "solve" and solve_active and visually_active:
